...
def process_document(file_path):
    ...
    print(f"Added {file_path} to processing queue - Size: {processing_queue.qsize()}")
...
def process_all_documents():
    ...
    for root, _, files in os.walk(DATA_DIR):
        for file in files:
            if file.endswith(('.pdf', '.docx', '.txt', '.jpg', '.jpeg', '.png', '.tiff', '.tif', '.bmp', '.gif')):
                document_count += 1
    ...
    print(f"Found {document_count} documents, queued {queued_count} for processing")
    return queued_count
